...
Para el componente de \textbf{front‑end}, al tratarse de un servicio de aplicación, fue necesario seleccionar entre los distintos tipos de despliegue que ofrece la plataforma \textbf{Render}. Las dos opciones principales para este propósito son el \textit{Web Service}, destinado a ejecutar aplicaciones con procesamiento en servidor, y el \textit{Static Site}, orientado al alojamiento de contenido generado por aplicaciones cliente. Ambos modelos resultan válidos para aplicaciones web, pero difieren sustancialmente en capacidad de ejecución, costes y configuración.

Render define un \textbf{Static Site} como un servicio optimizado para publicar archivos estáticos generados por una aplicación.\footnote{Documentación oficial: \url{https://render.com/docs/static-sites}} Una vez completada la fase de construcción del proyecto, el sistema toma los archivos resultantes —por ejemplo, los contenidos del directorio \texttt{dist} o \texttt{build} generados por \textit{Vite} o \textit{React}— y los despliega en una infraestructura distribuida globalmente. Este proceso habilita la entrega de contenido mediante una \textbf{red de distribución de contenidos (CDN)} integrada, lo que garantiza una descarga rápida y estable desde ubicaciones cercanas al usuario final. Además, Render gestiona automáticamente aspectos como el HTTPS, la asignación de dominio, las redirecciones y la caché de contenido, simplificando una configuración que, en otros entornos, requeriría varios servicios coordinados.

Se optó por este tipo de servicio porque el \textbf{front‑end} de \textbf{Muncher} es una aplicación de página única (\textit{Single Page Application, SPA}) que no requiere ejecución de código en el servidor. Bajo este modelo, basta con almacenar y servir los archivos del cliente compilado (HTML, CSS, JavaScript y recursos estáticos), lo que reduce los costes de infraestructura al eliminar cualquier necesidad de instancias o procesos persistentes. La combinación de simplicidad en el despliegue y rendimiento mediante CDN hace del \textit{Static Site} la opción más eficiente y adecuada para la naturaleza del proyecto.